...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!userId || !isSuperAdmin(callerRole)) {
    return NextResponse.json({ error: "Forbidden: only the main admin can delete events." }, { status: 403 });
  }
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/events/${id}`, {
      method: "DELETE",
      headers: { "x-user-role": callerRole },
    });
    ...
    console.error("[admin/events:delete] Request failed:", err);
