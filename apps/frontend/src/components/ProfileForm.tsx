...
interface ProfileFormProps {
  ...
  /** Show the "Age Status" (18+ / under 18) field. Used for volunteers. */
  showAgeStatus?: boolean;
  /** Show the "About Me" section. Defaults to true. */
  showAboutMe?: boolean;
  onSave?: () => Promise<any>;
}
...
export default function ProfileForm({
  ...
  // Keep local state in sync when Clerk finishes loading / props change.
  useEffect(() => {
    const synced = {
      firstName: initialData.firstName,
      lastName: initialData.lastName,
    ...
    setOriginalAboutMe(initialData.aboutMe || "");
  }, [
  ...
  const buildProfilePayload = (extra: Record<string, any> = {}) => {
    ...
    if (showAgeStatus) payload.isAdult = formData.isAdult;
    return payload;
  };
  ...
  const handleSaveAbout = async () => {
    ...
    try {
      const response = await fetch("/api/profile", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(buildProfilePayload({ aboutMe })),
      });
    ...
    <div className={styles.formBox}>
      ...
      <div className={styles.sectionBlock}>
        ...
        {!isEditing && (
          <div className={styles.viewGrid}>
            ...
            {showAgeStatus && (
              <div className={styles.viewField}>
                <span className={styles.viewLabel}>Age Status</span>
                <span className={styles.viewValue}>{formData.isAdult ? "18 and up" : "Under 18"}</span>
              </div>
            )}
          </div>
        )}
      ...
      {showAboutMe && (
        <div className={styles.sectionBlock}>
          ...
          {aboutStatus.type && (
            <div
              className={`${styles.statusMessage} ${aboutStatus.type === "success" ? styles.success : styles.error}`}
            >
              {aboutStatus.message}
            </div>
          )}
